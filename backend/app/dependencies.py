...
async def get_review_repo(session: AsyncSession = Depends(get_db)) -> ReviewItemRepository:
    return ReviewItemRepository(session)

async def get_study_session_repo(session: AsyncSession = Depends(get_db)) -> StudySessionRepository:
...
async def get_cost_guard_service(session: AsyncSession = Depends(get_db)) -> CostGuardService:
