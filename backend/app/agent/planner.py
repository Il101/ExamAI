...
class CoursePlanner:
    ...
    async def make_plan(self, state: AgentState) -> List[PlanStep]:
        ...
        print(f"[Planner] LLM response received. Tokens: {response.tokens_input}/{response.tokens_output}")
...
Return ONLY valid JSON, no markdown code blocks, no explanations.'''
    ...
    def _validate_plan(self, plan: List[PlanStep]):
        ...
        if len(plan) < 3:
            print(f"[Planner] Warning: Only {len(plan)} topics generated (unusually low)")
