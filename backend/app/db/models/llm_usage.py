...
class LLMUsageLogModel(Base):
    ...
    study_material_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), nullable=True) # Assuming study_materials table exists but not strictly enforcing FK here to avoid circular deps if not needed, or I should check if StudyMaterialModel exists.
    ...
    request_metadata: Mapped[Optional[dict[str, Any]]] = mapped_column(JSONB, default={})
