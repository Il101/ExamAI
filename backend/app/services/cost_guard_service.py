...
class CostGuardService:
    ...
    async def check_budget(
        self, user: User, estimated_cost: float, apply_buffer: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Get daily limit
        daily_limit = self.DAILY_LIMITS.get(
        ...
        if estimated_cost <= usable_budget:
            ...
            overage_percentage = (
                (overage / remaining * 100) if remaining > 0 else float("inf")
            )
    ...
    async def handle_actual_cost_overage(
        self, user: User, estimated_cost: float, actual_cost: float
    ) -> Dict[str, Any]:
        """
        ...
        overage_percentage = (
            (overage / estimated_cost * 100) if estimated_cost > 0 else float("inf")
        )

        policy = self.OVERAGE_POLICY.get(
            user.subscription_plan, self.OVERAGE_POLICY["free"]
        )
    ...
    async def get_usage_stats(self, user_id: UUID, days: int = 30) -> Dict[str, Any]:
        ...
        stmt = select(
            ...
            func.sum(
                LLMUsageLogModel.input_tokens + LLMUsageLogModel.output_tokens
            ).label("total_tokens"),
        ...
        return {
            ...
            "avg_cost_per_operation": (
                total_cost / operations_count if operations_count > 0 else 0.0
            ),
        }
