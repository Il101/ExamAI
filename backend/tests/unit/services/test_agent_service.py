...
class TestAgentService:
    ...
    async def test_generate_exam_content_success(self, service, mock_exam_repo, mock_agent, mock_cost_guard):
        # Arrange
        ...
        mock_exam_repo.update.return_value = exam  # Ensure update returns the exam object
    ...
    async def test_generate_exam_budget_exceeded(self, service, mock_exam_repo, mock_cost_guard):
        # Arrange
        user = User(email="test@example.com", full_name="Test")
        exam = Exam(
            user_id=user.id, 
        ...
        mock_exam_repo.get_by_user_and_id.return_value = exam
        mock_cost_guard.check_budget.return_value = {"allowed": False, "reason": "Limit reached"}
