...
class TestPlanAndExecuteAgent:
    ...
    async def test_run_success(self, mock_llm_provider):
        ...
        # Setup planner mock
        plan_steps = [
            PlanStep(id=1, title="Topic 1", description="Description for topic 1 that is long enough.", priority=1, estimated_paragraphs=3, dependencies=[]),
            PlanStep(id=2, title="Topic 2", description="Description for topic 2 that is long enough.", priority=1, estimated_paragraphs=3, dependencies=[1])
        ]
