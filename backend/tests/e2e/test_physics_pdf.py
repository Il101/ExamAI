...
class TestPhysicsPDF:
    ...
    async def test_physics_pdf_exam_generation(self):
        ...
        """
        # Read PDF file
        pdf_path = Path("/Users/iliazarikov/Documents/Python Projects/ExamAI/VO Physik ges 2022.pdf")
        ...
        try:
            ...
            with open(pdf_path, 'rb') as file:
                ...
                print(f"[E2E-Physics] Extracted {len(text_content)} chars from {pages_to_read}/{total_pages} pages")
        ...
        async with AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
            ...
            try:
                ...
                print(f"[E2E-Physics] Creating exam with {len(text_content)} chars of content")
                ...
                exam_data = {
                    ...
                    "exam_type": "written",
                    "level": "bachelor",
                    "original_content": text_content[:10000]  # Limit to 10k chars for speed
                }
                
                create_response = await client.post("/api/v1/exams/", json=exam_data, headers=headers)
                ...
                # Trigger generation
                gen_response = await client.post(f"/api/v1/exams/{exam_id}/generate", headers=headers)
                ...
                # Verify results
                get_response = await client.get(f"/api/v1/exams/{exam_id}", headers=headers)
                final_exam = get_response.json()
                
                print(f"\n[E2E-Physics] RESULTS:")
                print(f"  Status: {final_exam['status']}")
                print(f"  Topics generated: {len(final_exam['topics'])}")
                print(f"  Token usage: {final_exam['token_count_input']} → {final_exam['token_count_output']}")
                ...
                for i, topic in enumerate(final_exam["topics"], 1):
                    print(f"  {i}. {topic['topic_name']} (difficulty: {topic['difficulty_level']}/5)")
                
                # Show preview of first topic
                if final_exam["topics"]:
                    first_topic = final_exam["topics"][0]
                    preview = first_topic["content"][:300] if first_topic["content"] else "No content"
